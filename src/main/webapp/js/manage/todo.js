...
/* Todo管理 */
// 查询
function todoGetList() {
    console.log('前端函数执行中：function todoGetList()')
    // 先得到最大页数并渲染到分页栏里
    $.ajax({
        url: 'todo/maxpage',
        type: "POST",
        ...
        data: {
            ...
            'idLike': $('#todo-id-like').val(),
            'userLike': $('#todo-user-like').val(),
            'titleLike': $('#todo-title-like').val(),
            'contentLike': $('#todo-content-like').val(),
            'tagLike': allTodoTag,
            'classifyLike': $('#todo-classify-like').val(),
            'istopLike': allTodoIstop,
            'iscircleLike': allTodoIscircle,
            'iscomplete': allTodoIscomplete
        },
        success: function (res) {
            ...
            for (var i = 1; i <= maxPage; i++) {
                paginationHtml += `<li><a href="#" onclick="changeAllPageIndexTodo(${i})">${i}</a></li>`;
            }
            paginationHtml += `<li>
        ...
        success: function (res) {
            ...
            allTodoList.forEach((item, index) => {
                tbodyHtml += `<tr>
                                ...
                                <td>${item.content}</td>`;
                //翻译枚举/数字类型
                tbodyHtml += `  <td>${translationEnumTag(item.tag.toLowerCase())}</td>`
                tbodyHtml += `  <td>${item.classify}</td>`
                //翻译枚举类型/数字
                tbodyHtml += `  <td>${translationNumIstop(item.istop)}</td>
                                <td>${translationEnumIscircle(item.iscircle.toLowerCase())}</td>
                                <td>${translationNumIscomplete(item.iscomplete)}</td>`
                tbodyHtml += `
                                <td>
                                    <button type="button" class="btn btn-warning" onclick="todoDialogArise(2,${index})">修改</button>
                                    <button type="button" class="btn btn-danger" onclick="todoDel('${item.id}')">删除</button>
                                </td>
                            </tr>`;
            });
            $('#todoTbody').html(tbodyHtml);
        },
        error: function (err) {
            console.log('请求失败 err = ', err)
            console.log('失败参数=', {
...
// 点击分页改变页码，重新获取列表
function changeAllPageIndexTodo(i) {
    console.log('前端函数执行中：function changeAllPageIndexTodo(i)')
    allPageIndex = i;
    todoGetList();
}

// 下拉列表筛选
function changeTodoTag(element) {
    console.log('前端函数执行中：function changeTodoTag(element)');
    var todoTagText = $(element).text();
    console.log(todoTagText);
    if (todoTagText === '全部') {
        allTodoTag = '';
    }
    else if (todoTagText === '重要且紧急') {
        allTodoTag = 'red';
    }
    else if (todoTagText === '重要不紧急') {
        allTodoTag = 'yellow';
    }
    else if (todoTagText === '不重要但紧急') {
        allTodoTag = 'blue';
    }
    else if (todoTagText === '不重要不紧急') {
        allTodoTag = 'default';
    }
    todoGetList();
}
function changeTodoIstop(element) {
    console.log('前端函数执行中：function changeTodoIstop(element)');
    var todoIstopText = $(element).text();
    console.log(todoIstopText);
    if (todoIstopText === '全部') {
        allTodoIstop = '';
    }
    else if (todoIstopText === '是') {
        allTodoIstop = 2;
    }
    else if (todoIstopText === '否') {
        allTodoIstop = 1;
    }
    todoGetList();
}
function changeTodoIscircle(element) {
    console.log('前端函数执行中：function changeTodoIscircle(element)');
    var todoIscircleText = $(element).text();
    console.log(todoIscircleText);
    if (todoIscircleText === '全部') {
        allTodoIscircle = '';
    }
    else if (todoIscircleText === '日常任务') {
        allTodoIscircle = 'day';
    }
    else if (todoIscircleText === '周常任务') {
        allTodoIscircle = 'week';
    }
    else if (todoIscircleText === '支线任务') {
        allTodoIscircle = 'default';
    }
    todoGetList();
}
function changeTodoIscomplete(element) {
    console.log('前端函数执行中：function changeTodoIscomplete(element)');
    var todoIscompleteText = $(element).text();
    console.log(todoIscompleteText);
    if (todoIscompleteText === '全部') {
        allTodoIscomplete = '';
    }
    else if (todoIscompleteText === '是') {
        allTodoIscomplete = 2;
    }
    else if (todoIscompleteText === '否') {
        allTodoIscomplete = 1;
    }
    todoGetList();
}
...
function todoResult() {
    ...
    $('#todo-user-like').val('');
    $('#todo-title-like').val('');
    $('#todo-content-like').val('');
    $('#todo-classify-like').val('');
